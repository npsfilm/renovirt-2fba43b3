...
const Settings = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('customer_profiles').select('marketing_emails, sms_notifications, order_updates').eq('user_id', user.id).single();
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.functions.invoke('send-account-deletion-request', {
        body: {
          userId: user?.id,
          email: user?.email,
  ...
  return (
    <MobileLayout>
      {!isMobile && (
        <PageHeader title="Einstellungen" subtitle="Verwalten Sie Ihre Benachrichtigungen und Kontoeinstellungen" />
      )}
      <div className="space-y-6 p-6">
        {isMobile && (
          <div className="mb-6">
            <h1 className="text-2xl font-semibold text-foreground">Einstellungen</h1>
            <p className="text-muted-foreground">Verwalten Sie Ihre Benachrichtigungen und Kontoeinstellungen</p>
          </div>
        )}
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch id="sms-notifications" checked={notifications.smsNotifications} onCheckedChange={value => handleNotificationChange('smsNotifications', value)} />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch id="marketing-emails" checked={notifications.marketingEmails} onCheckedChange={value => handleNotificationChange('marketingEmails', value)} />
            </div>

            <div className="pt-4 border-t">
              <Button onClick={handleSaveNotifications} disabled={saveNotificationsMutation.isPending}>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="p-4 border border-red-200 rounded-lg bg-red-50">
              ...
              <p className="text-sm text-red-700 mb-4">Das Löschen Ihres Kontos kann nicht rückgängig gemacht werden. 
Alle Ihre Daten und Bestellungen werden permanent entfernt.</p>
              <div className="space-y-3">
                <div>
                  <Label htmlFor="delete-confirmation" className="text-red-900">
                    Geben Sie "Löschen" ein, um zu bestätigen:
                  </Label>
                  <Input id="delete-confirmation" value={deleteConfirmation} onChange={e => setDeleteConfirmation(e.target.value)} placeholder="Löschen" className="mt-1" />
                </div>
                <Button variant="destructive" size="sm" onClick={handleDeleteAccount} disabled={deleteConfirmation !== 'Löschen'}>
                  Konto löschen
                </Button>
              </div>
            </div>
