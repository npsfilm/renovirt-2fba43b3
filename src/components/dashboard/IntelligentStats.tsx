...
const IntelligentStats = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: orders,
        error
      } = await supabase.from('orders').select('status, image_count, created_at').eq('user_id', user.id).neq('payment_flow_status', 'draft');
  ...
  return <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      return <Card key={index} className="border-0 bg-card/50 backdrop-blur-sm hover:bg-card/80 transition-all duration-300 hover:shadow-md">
